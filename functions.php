...
function moodle_session_DB(&$dbo, $dbname, $tblprefix, $dbtype = "pdomysql") {
    if ($dbtype == "pdomysql") {
        $sql = "SELECT COUNT(id)" .
            " FROM {$dbname}.{$tblprefix}user WHERE lastaccess >= (UNIX_TIMESTAMP() - 1800)";
...
function moodle_session_files($multi = false) {
    global $CFG;
