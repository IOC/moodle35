 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_cre_testcase extends advanced_testcase {

    /**
     * Tests for format_cre::get_section_name method with default section names.
     */
    public function test_get_section_name() {
        ...
        $numsections = 5;
        $course = $generator->create_course(array('numsections' => $numsections, 'format' => 'cre'),
    ...
    /**
     * Tests for format_cre::get_section_name method with modified section names.
     */
    ...
    /**
     * Tests for format_cre::get_default_section_name.
     */
    public function test_get_default_section_name() {
        ...
        foreach ($coursesections as $section) {
            if ($section->section == 0) {
                $sectionname = get_string('section0name', 'format_cre');
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            } else {
                $sectionname = get_string('sectionname', 'format_cre') . ' ' . $section->section;
                $this->assertEquals($sectionname, $courseformat->get_default_section_name($section));
            }
        }
    }
    ...
    public function test_update_inplace_editable() {
        ...
        $this->setUser($user);
        $course = $this->getDataGenerator()->create_course(array('numsections' => 5, 'format' => 'cre'),
            array('createsections' => true));
        $section = $DB->get_record('course_sections', array('course' => $course->id, 'section' => 2));

        // Call webservice without necessary permissions.
        try {
            core_external::update_inplace_editable('format_cre', 'sectionname', $section->id, 'New section name');
        ...
        $res = core_external::update_inplace_editable('format_cre', 'sectionname', $section->id, 'New section name');
    ...
    public function test_inplace_editable() {
        ...
        // Call callback format_cre_inplace_editable() directly.
        $tmpl = component_callback('format_cre', 'inplace_editable', array('sectionname', $section->id, 'Rename me again'));
    ...
    public function test_default_course_enddate() {
        ...
        $params = array('format' => 'cre', 'numsections' => 5, 'startdate' => 1445644800);
    ...
    public function test_get_view_url() {
        ...
        $generator = $this->getDataGenerator();
        $course1 = $generator->create_course(array('format' => 'cre'));
